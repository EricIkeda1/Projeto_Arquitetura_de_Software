...
def create_fig_linha(vendas, produtos):
    ...
    fig_linha.update_layout(title='Gráfico de Linha', xaxis_title='Mês', yaxis_title='Valor Total')
    return fig_linha
...
def create_fig_barras(vendas, produtos):
    ...
    fig_barras.update_layout(title='Gráfico de Barras', xaxis_title='Mês', yaxis_title='Quantidade')
    return fig_barras
...
def create_fig_barras_linha(produtos):
    ...
    for grupo in top_grupos:
        ...
        metas['meta'].append(1000 if grupo['total_vendido'] >= 1000 else None)
    ...
    fig_barras_linha.add_trace(go.Scatter(x=[grupo['grupo'] for grupo in top_grupos], 
                                          y=[meta for meta in metas['meta']],
                                          mode='lines',
                                          name='Meta 1000 Unidades'))

    fig_barras_linha.update_layout(title='Gráfico de Barras e Linha', xaxis_title='Grupo de Produto', yaxis_title='Quantidade Vendida')
